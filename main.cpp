...
typedef struct ImageSt {        /*金字塔每一层*/
	...
	float levelsigma;
	int levelsigmalength;//存储高斯卷积窗口的宽度(dim%2)
...
ImageOctaves* mag_pyr;//sqrt(dx*dx+dy*dy)
ImageOctaves* grad_pyr;//atan(dy/dx)
...
#define Im1R(ROW,COL) ((uchar*)(image1->imageData + image1->widthStep*(ROW)))[(COL)*3+2]
	...
	//读取图片
	if ((src = cvLoadImage("C:/Users/76760/Desktop/test1.jpg", 1)) == 0) 
		return -1;
...
#define TEMPMAT(ROW,COL) ((float *)(tempMat->data.fl + tempMat->step/sizeof(float) * (ROW)))[(COL)]
		...
		preblur_sigma = 1.0;//sqrt(2 - 4*INITSIGMA*INITSIGMA);      INITSIGMA=0.5	SIGMA sqrt(3)=1.6
...
ImageOctaves* BuildGaussianOctaves(CvMat* image)
{
	...
	for (i = 0; i < numoctaves; i++)
	{
		...
		if (DOUBLE_BASE_IMAGE_SIZE)
			octaves[i].subsample = pow(2, i) * 0.5;//下采样
		else
...
void ExtractFeatureDescriptors(int numoctaves, ImageOctaves* GaussianPyr)
{
	...
	for (int i = 0; i < GridSpacing; i++)
	{
		for (int j = 0; j < 2 * GridSpacing; ++j, ++j)
		{
			...
			printf("%f  %f\n", feat_grid[i * 2 * GridSpacing + j], feat_grid[i * 2 * GridSpacing + j + 1]);
		}
	...
	for (int i = 0; i < 4 * GridSpacing; i++)
	{
		for (int j = 0; j < 8 * GridSpacing; j += 2)
		{
			...
			printf("%f  %f\n", feat_samples[i * 8 * GridSpacing + j], feat_samples[i * 8 * GridSpacing + j + 1]);
		}
	}
